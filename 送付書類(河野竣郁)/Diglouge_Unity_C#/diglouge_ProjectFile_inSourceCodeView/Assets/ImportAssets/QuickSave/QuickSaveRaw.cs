...
namespace CI.QuickSave
{
    public class QuickSaveRaw
    {
        ...
        public static async Task SaveStringAcync(string filename, string content, QuickSaveSettings settings)
        {
            string contentToWrite;
            ...
            if (!await FileAccess.SaveStringAsync(filename, true, contentToWrite))
            {
                throw new QuickSaveException("Failed to write to file");
            }
        }
        ...
        /// <returns></returns>
        /// <exception cref="QuickSaveException"></exception>
        public static async Task SaveBytesAsync(string filename, byte[] content)
        {
            if (!await FileAccess.SaveBytesAsync(filename, true, content))
            {
                throw new QuickSaveException("Failed to write to file");
            }
        }
